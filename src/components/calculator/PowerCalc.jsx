...
const PowerCalc = () => {
  ...
  const calculate = () => {
    const { calculationType, systemType, voltage, voltageUnit, current, currentUnit, 
            impedance, impedanceUnit, resistance, resistanceUnit, apparentPower, apparentPowerUnit,
    ...
    switch (calculationType) {
      case 'voltage-current':
        switch (systemType) {
          case 'single-phase':
            activePower = voltageInV * currentInA * pf;
            formula = 'P = V × I × cos φ';
            break;
          case 'two-phase':
            activePower = 2 * voltageInV * currentInA * pf;
            formula = 'P = 2 × V × I × cos φ';
            break;
          case 'three-phase':
            activePower = Math.sqrt(3) * voltageInV * currentInA * pf;
            formula = 'P = √3 × V × I × cos φ';
            break;
          case 'dc':
            activePower = voltageInV * currentInA;
            formula = 'P = V × I';
            break;
        }
        break;
      ...
      case 'voltage-impedance':
        activePower = (voltageInV * voltageInV) / impedanceInOhm;
        formula = 'P = V² / Z';
        break;
        
      case 'voltage-resistance':
        activePower = (voltageInV * voltageInV) / resistanceInOhm;
        formula = 'P = V² / R';
        break;
        
      case 'current-impedance':
        activePower = currentInA * currentInA * impedanceInOhm;
        formula = 'P = I² × Z';
        break;
        
      case 'current-resistance':
        activePower = currentInA * currentInA * resistanceInOhm;
        formula = 'P = I² × R';
        break;
        
      case 'apparent-reactive':
        activePower = Math.sqrt(Math.pow(apparentPowerInVA, 2) - Math.pow(reactivePowerInVar, 2));
        formula = 'P = √(S² - Q²)';
        break;
        
      case 'apparent-only':
        activePower = apparentPowerInVA * pf;
        formula = 'P = S × cos φ';
        break;
        
      case 'reactive-only':
        activePower = reactivePowerInVar / Math.tan(Math.acos(pf));
        formula = 'P = Q / tan φ';
        break;
    }
    
    // Convertir resultado a la unidad seleccionada
    const displayActivePower = activePower / activePowerConversions[resultUnit];
    
    setResult({
      activePower: displayActivePower.toFixed(3),
      activePowerUnit: resultUnit,
      formula: formula,
      calculationType: calculationType,
      systemType: systemType,
      activePowerInW: activePower.toFixed(3)
    });
  };
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-semibold mb-6">Cálculo de Potencia Activa</h2>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          {(inputs.calculationType === 'apparent-reactive' || inputs.calculationType === 'apparent-only') && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Potencia Aparente
              </label>
          ...
          {(inputs.calculationType === 'apparent-reactive' || inputs.calculationType === 'reactive-only') && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Potencia Reactiva
              </label>
          ...
          {/* Factor de Potencia - Solo para sistemas AC y cálculos específicos */}
          {(inputs.calculationType === 'voltage-current' || inputs.calculationType === 'apparent-only' || inputs.calculationType === 'reactive-only') && inputs.systemType !== 'dc' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Factor de Potencia
              </label>
          ...
          {result ? (
            <>
              <div className="bg-blue-50 p-4 rounded-lg">
                <div className="text-sm text-blue-600 font-medium">Potencia Activa Calculada</div>
                <div className="text-2xl font-bold text-blue-900">{result.activePower} {result.activePowerUnit}</div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="text-xs text-gray-600 space-y-1">
                  <div>• Tipo: {result.calculationType.replace('-', ' / ').replace('voltage', 'tensión').replace('current', 'corriente').replace('impedance', 'impedancia').replace('resistance', 'resistencia').replace('apparent', 'potencia aparente').replace('reactive', 'potencia reactiva').replace('only', 'sólo')}</div>
                  ...
                  <div>• Valor base: {result.activePowerInW} W</div>
                </div>
              </div>
            </>
          ) : (
      ...
      <div className="mt-8 bg-blue-50 p-4 rounded-lg border-l-4 border-blue-500">
        ...
        <div className="text-blue-800 font-mono space-y-2 text-sm">
          {inputs.calculationType === 'voltage-current' && (
            <>
              ...
              <div>• Monofásico: P = V × I × cos φ</div>
              <div>• Bifásico: P = 2 × V × I × cos φ</div>
              <div>• Trifásico: P = √3 × V × I × cos φ</div>
              <div>• Corriente Continua: P = V × I</div>
            </>
          )}
          ...
          {inputs.calculationType === 'apparent-reactive' && (
            <>
              <div><strong>Potencia Aparente / Potencia Reactiva:</strong></div>
              <div>• P = √(S² - Q²)</div>
            </>
          )}
        ...
        <div className="text-sm text-blue-700 mt-3">
          P = Potencia Activa, V = Tensión, I = Corriente, S = Potencia Aparente, Q = Potencia Reactiva, Z = Impedancia, R = Resistencia, cos φ = Factor de potencia
        </div>
      </div>
    </div>
  );
