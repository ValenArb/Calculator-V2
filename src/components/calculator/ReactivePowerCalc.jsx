...
const ReactivePowerCalc = () => {
  ...
  const calculate = () => {
    const { calculationType, systemType, voltage, voltageUnit, current, currentUnit, 
            ...
            apparentPower, apparentPowerUnit, powerFactor, powerFactorType, resultUnit } = inputs;
    ...
    const apparentPowerInVA = apparentPower * apparentPowerConversions[apparentPowerUnit];
    ...
    // Calcular sen φ para algunos cálculos
    const sinPhi = Math.sqrt(1 - Math.pow(pf, 2));

    switch (calculationType) {
      case 'voltage-current':
        switch (systemType) {
          case 'single-phase':
            reactivePower = voltageInV * currentInA * sinPhi;
            formula = 'Q = V × I × sen φ';
            break;
          case 'two-phase':
            reactivePower = 2 * voltageInV * currentInA * sinPhi;
            formula = 'Q = 2 × V × I × sen φ';
            break;
          case 'three-phase':
            reactivePower = Math.sqrt(3) * voltageInV * currentInA * sinPhi;
            formula = 'Q = √3 × V × I × sen φ';
            break;
        }
        break;
        
      case 'voltage-impedance':
        const reactiveComponentZ = impedanceInOhm * sinPhi;
        reactivePower = (voltageInV * voltageInV) / reactiveComponentZ;
        formula = 'Q = V² / (Z × sen φ)';
        break;
        
      case 'voltage-resistance':
        const tanPhi = Math.tan(Math.acos(pf));
        reactivePower = (voltageInV * voltageInV * tanPhi) / resistanceInOhm;
        formula = 'Q = V² × tan φ / R';
        break;
        
      case 'current-impedance':
        const reactiveImpedance = impedanceInOhm * sinPhi;
        reactivePower = currentInA * currentInA * reactiveImpedance;
        formula = 'Q = I² × (Z × sen φ)';
        break;
        
      case 'current-resistance':
        const tanPhiCR = Math.tan(Math.acos(pf));
        reactivePower = currentInA * currentInA * resistanceInOhm * tanPhiCR;
        formula = 'Q = I² × R × tan φ';
        break;
        
      case 'active-apparent':
        reactivePower = Math.sqrt(Math.pow(apparentPowerInVA, 2) - Math.pow(activePowerInW, 2));
        formula = 'Q = √(S² - P²)';
        break;
        
      case 'active-only':
        const tanPhiAO = Math.tan(Math.acos(pf));
        reactivePower = activePowerInW * tanPhiAO;
        formula = 'Q = P × tan φ';
        break;
        
      case 'apparent-only':
        reactivePower = apparentPowerInVA * sinPhi;
        formula = 'Q = S × sen φ';
        break;
    }
    
    // Convertir resultado a la unidad seleccionada
    const displayReactivePower = reactivePower / reactivePowerConversions[resultUnit];
    
    setResult({
      reactivePower: displayReactivePower.toFixed(3),
      reactivePowerUnit: resultUnit,
      formula: formula,
      calculationType: calculationType,
      systemType: systemType,
      reactivePowerInVar: reactivePower.toFixed(3)
    });
  };
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-semibold mb-6">Cálculo de Potencia Reactiva</h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            >
              ...
              <option value="apparent-only">Potencia Aparente</option>
            </select>
          </div>
          ...
          {(inputs.calculationType === 'active-apparent' || inputs.calculationType === 'active-only') && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Potencia Activa
              </label>
          ...
          {/* Potencia Aparente - Para cálculos que requieren potencia aparente */}
          {(inputs.calculationType === 'active-apparent' || inputs.calculationType === 'apparent-only') && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Potencia Aparente
              </label>
          ...
          {(inputs.calculationType === 'active-apparent' || inputs.calculationType === 'voltage-current' || inputs.calculationType === 'voltage-impedance' || inputs.calculationType === 'voltage-resistance' || inputs.calculationType === 'current-impedance' || inputs.calculationType === 'current-resistance' || inputs.calculationType === 'active-only' || inputs.calculationType === 'apparent-only') && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Factor de Potencia
              </label>
          ...
          {result ? (
            <>
              <div className="bg-purple-50 p-4 rounded-lg">
                <div className="text-sm text-purple-600 font-medium">Potencia Reactiva Calculada</div>
                <div className="text-2xl font-bold text-purple-900">{result.reactivePower} {result.reactivePowerUnit}</div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="text-xs text-gray-600 space-y-1">
                  <div>• Tipo: {result.calculationType.replace('-', ' / ').replace('voltage', 'tensión').replace('current', 'corriente').replace('impedance', 'impedancia').replace('resistance', 'resistencia').replace('active', 'potencia activa').replace('apparent', 'potencia aparente').replace('only', 'sólo')}</div>
                  <div>• Sistema: {result.systemType === 'single-phase' ? 'Monofásico' :
                                    result.systemType === 'two-phase' ? 'Bifásico' : 'Trifásico'}</div>
                  <div>• Valor base: {result.reactivePowerInVar} var</div>
                </div>
              </div>
            </>
          ) : (
        ...
        <div className="text-blue-800 font-mono space-y-2 text-sm">
          {inputs.calculationType === 'voltage-current' && (
            <>
              ...
              <div>• Monofásico: Q = V × I × sen φ</div>
              <div>• Bifásico: Q = 2 × V × I × sen φ</div>
              <div>• Trifásico: Q = √3 × V × I × sen φ</div>
            </>
          )}
          ...
          {inputs.calculationType === 'active-apparent' && (
            <>
              <div><strong>Potencia Activa / Potencia Aparente:</strong></div>
              <div>• Q = √(S² - P²)</div>
            </>
          )}
          ...
          {inputs.calculationType === 'apparent-only' && (
            <>
              <div><strong>Potencia Aparente:</strong></div>
              <div>• Q = S × sen φ</div>
            </>
          )}
        </div>
        <div className="text-sm text-blue-700 mt-3">
          Q = Potencia Reactiva, V = Tensión, I = Corriente, P = Potencia Activa, S = Potencia Aparente, Z = Impedancia, R = Resistencia, cos φ = Factor de potencia, sen φ = sen(φ), tan φ = tan(φ)
        </div>
      </div>
    </div>
  );
...
export default ReactivePowerCalc;