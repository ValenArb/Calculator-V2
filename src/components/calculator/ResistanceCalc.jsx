...
const ResistanceCalc = () => {
  ...
  const calculate = () => {
    const { calculationType, systemType, voltage, voltageUnit, current, currentUnit, 
            power, powerUnit, impedance, impedanceUnit, powerFactor, powerFactorType, resultUnit } = inputs;
    ...
    switch (calculationType) {
      case 'voltage-current':
        resistance = voltageInV / currentInA;
        formula = 'R = V / I';
        break;
      ...
      case 'voltage-power':
        ...
        switch (systemType) {
          case 'single-phase':
            resistance = (voltageInV * voltageInV) / effectivePower;
            formula = 'R = V² / P';
            break;
          case 'two-phase':
            resistance = (voltageInV * voltageInV) / (effectivePower / 2);
            formula = 'R = V² / (P/2)';
            break;
          case 'three-phase':
            resistance = (voltageInV * voltageInV) / (effectivePower / 3);
            formula = 'R = V² / (P/3)';
            break;
          case 'dc':
            resistance = (voltageInV * voltageInV) / powerInW;
            formula = 'R = V² / P';
            break;
        }
        break;
      ...
      case 'current-power':
        resistance = powerInW / (currentInA * currentInA);
        formula = 'R = P / I²';
        break;
        
      case 'impedance':
        // Para DC o circuitos puramente resistivos, R = Z
        resistance = impedanceInOhm;
        formula = 'R = Z (circuito resistivo)';
        if (systemType !== 'dc' && powerFactor < 1) {
          resistance = impedanceInOhm * pf;
          formula = 'R = Z × cos φ';
        }
        break;
    }
    ...
    const displayResistance = resistance / resistanceConversions[resultUnit];
    
    setResult({
      resistance: displayResistance.toFixed(3),
      resistanceUnit: resultUnit,
      formula: formula,
      calculationType: calculationType,
      systemType: systemType,
      resistanceInOhms: resistance.toFixed(3)
    });
  };
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-semibold mb-6">Cálculo de Resistencia</h2>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          {inputs.calculationType === 'impedance' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Impedancia
              </label>
          ...
          {/* Factor de Potencia - Solo para cálculos con potencia y no DC */}
          {(inputs.calculationType === 'voltage-power' || inputs.calculationType === 'impedance') && inputs.systemType !== 'dc' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Factor de Potencia
              </label>
          ...
          {result ? (
            <>
              <div className="bg-blue-50 p-4 rounded-lg">
                <div className="text-sm text-blue-600 font-medium">Resistencia Calculada</div>
                <div className="text-2xl font-bold text-blue-900">{result.resistance} {result.resistanceUnit}</div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="text-xs text-gray-600 space-y-1">
                  <div>• Tipo: {result.calculationType.replace('-', ' / ').replace('voltage', 'tensión').replace('current', 'corriente').replace('power', 'potencia').replace('impedance', 'impedancia')}</div>
                  ...
                  <div>• Valor base: {result.resistanceInOhms} Ω</div>
                </div>
              </div>
            </>
          ) : (
        ...
        <div className="text-blue-800 font-mono space-y-2 text-sm">
          ...
          {inputs.calculationType === 'voltage-power' && (
            <>
              ...
              <div>• Monofásico: R = V² / P</div>
              <div>• Bifásico: R = V² / (P/2)</div>
              <div>• Trifásico: R = V² / (P/3)</div>
              <div>• Corriente Continua: R = V² / P</div>
            </>
          )}
          ...
          {inputs.calculationType === 'impedance' && (
            <>
              <div><strong>Impedancia:</strong></div>
              <div>• Circuito resistivo puro: R = Z</div>
              <div>• Con factor de potencia: R = Z × cos φ</div>
            </>
          )}
        </div>
        <div className="text-sm text-blue-700 mt-3">
          R = Resistencia, V = Tensión, I = Corriente, P = Potencia, Z = Impedancia, cos φ = Factor de potencia
        </div>
      </div>
    </div>
  );
